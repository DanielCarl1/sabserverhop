...
local AutoTeleportEnabled = false -- New feature
local AutoSaveEnabled = true -- New feature

-------------------------------------------------
-- CONFIGURATION SYSTEM
-------------------------------------------------
local ConfigFile = "synthzxshub_config.json"
local function saveSettings()
    if not AutoSaveEnabled then return end
    local settings = {
        AutoJoinerEnabled = AutoJoinerEnabled,
        AutoTeleportEnabled = AutoTeleportEnabled,
        AutoSaveEnabled = AutoSaveEnabled
    }
    local success, content = pcall(function() return HttpService:JSONEncode(settings) end)
    if success then
...
local function loadSettings()
    if isfile(ConfigFile) then
        ...
        if success then
            ...
            AutoJoinerEnabled = decoded.AutoJoinerEnabled or false
            _G.AutoJoinerEnabled = AutoJoinerEnabled
            AutoTeleportEnabled = decoded.AutoTeleportEnabled or false
            AutoSaveEnabled = decoded.AutoSaveEnabled or true
        end
    end
end
loadSettings()
...
Watermark.Text = "ΓÜí synthzxshub (Custom)"
...
AutoExitButton.Text = "≡ƒ¢æ Auto Exit: OFF"
...
AutoJoinerButton.Text = "≡ƒÜÇ Auto Join: " .. (AutoJoinerEnabled and "ON" or "OFF")
...
-------------------------------------------------
-- AUTO TELEPORT TO RARE
-------------------------------------------------
local AutoTeleportButton = Instance.new("TextButton")
AutoTeleportButton.Name = "AutoTeleportButton"
AutoTeleportButton.Size = UDim2.new(0, 150, 0, 24)
AutoTeleportButton.Position = UDim2.new(1, -160, 0, 158)
AutoTeleportButton.BackgroundColor3 = Color3.fromRGB(15, 20, 30)
AutoTeleportButton.BackgroundTransparency = 0.2
AutoTeleportButton.BorderSizePixel = 0
AutoTeleportButton.Text = "≡ƒì▓ Auto TP: " .. (AutoTeleportEnabled and "ON" or "OFF")
AutoTeleportButton.TextColor3 = Color3.fromRGB(255, 200, 50)
AutoTeleportButton.TextSize = 12
AutoTeleportButton.Font = Enum.Font.GothamBold
AutoTeleportButton.Parent = ScreenGui
Instance.new("UICorner", AutoTeleportButton).CornerRadius = UDim.new(0, 6)
Instance.new("UIStroke", AutoTeleportButton).Color = Color3.fromRGB(200, 150, 0)
...
AutoSaveButton.Size = UDim2.new(0, 150, 0, 24)
AutoSaveButton.Position = UDim2.new(1, -160, 0, 186)
AutoSaveButton.BackgroundColor3 = Color3.fromRGB(15, 20, 30)
AutoSaveButton.BackgroundTransparency = 0.2
AutoSaveButton.BorderSizePixel = 0
AutoSaveButton.Text = "≡ƒæ¥ Auto Save: " .. (AutoSaveEnabled and "ON" or "OFF")
AutoSaveButton.TextColor3 = Color3.fromRGB(200, 150, 255)
AutoSaveButton.TextSize = 12
AutoSaveButton.Font = Enum.Font.GothamBold
AutoSaveButton.Parent = ScreenGui
Instance.new("UICorner", AutoSaveButton).CornerRadius = UDim.new(0, 6)
Instance.new("UIStroke", AutoSaveButton).Color = Color3.fromRGB(150, 50, 200)
...
AutoExitButton.MouseButton1Click:Connect(function()
    ...
    AutoExitButton.Text = "≡ƒ¢æ Auto Exit: " .. (autoExitEnabled and "ON" or "OFF")
end)
    ...
    task.spawn(function()
        ...
        if #servers > 0 then
            ...
            if AutoJoinerEnabled then
                HopButton.Text = "ΓÅ│ Retrying Hop..."
                task.wait(2)
                isHopping = false
            ...
            HopButton.Text = "Γ¥î No Servers Found"
...
local function performServerCheck()
    local currentGen, rareItemObject = calculateServerGen()
    GenLabel.Text = "Curr Gen: $" .. tostring(math.floor(currentGen/1000000)) .. "M/s"
    
    if rareItemObject and AutoTeleportEnabled then
        local targetPos = nil
        if rareItemObject:IsA("BasePart") then
            targetPos = rareItemObject.CFrame
        elseif rareItemObject:IsA("Model") and rareItemObject.PrimaryPart then
            targetPos = rareItemObject.PrimaryPart.CFrame
        elseif rareItemObject:IsA("Model") then
            targetPos = rareItemObject:GetModelCFrame()
        end
        
        if targetPos then
            pcall(function()
                LocalPlayer.Character:SetPrimaryPartCFrame(targetPos + Vector3.new(0, 3, 0))
            end)
        end
    end

    if AutoJoinerEnabled then
        if rareItemObject then
            AutoJoinerEnabled = false
            _G.AutoJoinerEnabled = false
            AutoJoinerButton.Text = "≡ƒÆÄ RARE: " .. string.upper(rareItemObject.Name)
            AutoJoinerButton.TextColor3 = Color3.fromRGB(255, 215, 0)
            saveSettings()
            
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "≡ƒÆÄ RARE ITEM FOUND!",
                Text = "Found: " .. rareItemObject.Name,
                Duration = 15,
            })
        elseif currentGen >= TARGET_GEN then
            AutoJoinerEnabled = false
            _G.AutoJoinerEnabled = false
            AutoJoinerButton.Text = "Γ£à TARGET FOUND"
            AutoJoinerButton.TextColor3 = Color3.fromRGB(0, 255, 0)
            saveSettings()
            
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "≡ƒÆÄ Target Reached!",
                Text = "Server Generation: $" .. math.floor(currentGen/1000000) .. "M/s",
                Duration = 10,
            })
        else
            AutoJoinerButton.Text = "ΓÅ│ Searching..."
            task.wait(2)
            hopToServer()
        end
    end
...
task.spawn(function()
    while ScreenGui.Parent do
        ...
        local color = ping < 80 and "≡ƒƒó" or (ping < 150 and "≡ƒƒí" or "≡ƒö┤")
